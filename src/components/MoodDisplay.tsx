...
const MoodDisplay: React.FC<MoodDisplayProps> = ({ selectedDate, entries }) => {
  ...
  return (
    <div className="space-y-3">
      <h3 className="text-lg font-semibold text-slate-700 dark:text-slate-200 mb-2 text-center">
      ...
      <ul className="space-y-3">
        {entries.map(entry => (
          <li 
            key={entry.id} 
            className="p-3 rounded-lg shadow-sm border-l-4 bg-white dark:bg-slate-800 transition-all duration-200 hover:shadow-md"
            style={{ borderColor: MOOD_COLORS[entry.mood] }}
          >
            <div className="flex items-start space-x-2.5">
              <span className="text-2xl">{MOOD_EMOJIS[entry.mood]}</span>
              <div className="flex-grow">
                <p className="font-semibold text-slate-800 dark:text-slate-100 text-sm">{getMoodText(entry.mood)}</p>
                {entry.note && <p className="text-xs text-slate-600 dark:text-slate-300 mt-0.5 whitespace-pre-wrap break-words">{entry.note}</p>}
                {entry.tags && entry.tags.length > 0 && (
                  <div className="mt-1.5 flex flex-wrap gap-1.5">
                    {entry.tags.map(tag => (
                      <span 
                        key={tag} 
                        className="px-2 py-0.5 text-xs font-medium bg-sky-100 text-sky-700 dark:bg-sky-700 dark:text-sky-100 rounded-full"
                      >
                        {tag}
                      </span>
                    ))}
